...
export function AegisScreen({ children }: { children: ReactNode }) {
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ color: CHARCOAL, background: CREAM }}>
      <Backdrop />
      <div className="relative z-10 mx-auto flex h-full w-full max-w-[440px] flex-col px-5 pt-[max(20px,env(safe-area-inset-top))] pb-[max(18px,env(safe-area-inset-bottom))] sm:px-6">
        {children}
      </div>
    </div>
  );
